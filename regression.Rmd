...
Adjust for multiple hypothesis testing with Benjamini-Hochberg (incorrect):
...
pvals_long <- pvals_long %>%
  mutate(pval_adj = p.adjust(pval_list, method = "BH"))
```
...
for (i in 1:nrow(imin)) {
  g <- plot_stim_response(stim_events1$worm[imin[i, 1]],
                          stim_events1$vol[imin[i, 1]],
                          colnames(pval_mat)[imin[i, 2]], sec_before = 10)
  print(g)
}
```
